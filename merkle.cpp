...
void reverseStr(string& str)
{
    ...
    // Revert characters from two limits
    for (int i = 0; i < n / 2; i++)
        swap(str[i], str[n - i - 1]);
}

// Function returns the double SHA256 hash of the inputs
string to_hash(string a, string b) {
    string adec, bdec, nodes, encoded, digest;
   
    // Decoding both inputs 
    StringSource ssa(a, true,
        new HexDecoder(
            new StringSink(adec)
        ) // HexDecoder
    ); // StringSource
    
    StringSource ssb(b, true,
        new HexDecoder(
            new StringSink(bdec)
        ) // HexDecoder
    ); // StringSource

    // Reversion is necessary due to little endian
    reverseStr(adec);
    reverseStr(bdec);

    // Concatenates the decoded inputs
    nodes = adec + bdec;

    SHA256 hash;

    // Bitcoin implements double SHA256 hashing
    hash.Update((const byte*)nodes.data(), nodes.size());
...
// Function returns the merkle root of a vector
vector<string> merkleRoot(vector<string> hashlist) {
    // Return variable
    vector<string> resp;
    
    // Loop the vector hashing the elements 2 by 2
    for(int i = 0; i < hashlist.size()-1; i+=2){
        resp.push_back(to_hash(hashlist[i], hashlist[i+1]));
    }

    // If the vector's size is odd, hash the last element with itself
    if(hashlist.size() % 2 != 0){
        resp.push_back(to_hash(hashlist[hashlist.size()-1], hashlist[hashlist.size()-1]));
    }

    if(resp.size() != 1) {
        return merkleRoot(resp);
    }
    return resp;
}
...
int main(int argc, char* argv[]) {
    // https://www.blockchain.com/btc/block/0000000000000000e067a478024addfecdc93628978aa52d91fabd4292982a50
